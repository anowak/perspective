...
struct t_udf_registry {
    ...
    void load_plugins_from_env() {
        ...
        // Fallback registration for environments that cannot dynamically load
        // shared libraries (e.g. the WASM build). This mirrors the sample
        // reducer provided by the `udf-plugin` example so the UDF path can be
        // exercised even when `dlopen` is unavailable.
        {
            std::lock_guard<std::mutex> lock(m_mutex);
